...
| `OSPS-QA-05.01` | Active version control must not contain generated executable artifacts. The complete tracked tree is applicable. | Met | `scripts/verify-source-artifacts.sh` accepts reviewable source plus one exact digest-bound documentation image and no generated executable; its disposable negative rehearsal rejects executable headers and executable-mode files outside reviewed scripts. |
| `OSPS-QA-05.02` | Active version control must not contain unreviewable binary artifacts. The complete tracked tree is applicable. | Met | The same source gate admits only the exact size-, digest-, header-, path-, and mode-bound README PNG, rejects a changed image and every other NUL-bearing, invalid UTF-8, disguised executable, or unreviewable binary, and is exercised by exact-head and exact-`main` CI. |
