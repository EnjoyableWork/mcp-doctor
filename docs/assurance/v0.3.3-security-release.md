...
This non-sensitive record closes two distinct CWE-400 advisories on 2026-08-18.
It records only public commits, pull requests, workflows, release artifacts,
advisories, aggregate repository-control state, and value-free
credential-inventory outcomes. It contains no private reproducer, endpoint,
credential, payload, schema content, tool argument, result, or
advisory-workspace material.

## Scope and outcome

The `v0.3.3` release coordinated two independently bounded corrections:
...
A separately reviewed exact-byte recovery path handled the partial release.
...
focused release-policy tests passed. The first complete `scripts/check.sh` gate
then stopped when one organization-policy assertion still required obsolete
repository-authority wording; the same stale assertion was corrected in all
three owning policy suites on new source, and their focused tests passed.
Neither failed source tree was rerun unchanged.
