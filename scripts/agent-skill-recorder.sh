...
agent_recorder_mode=${MCP_DOCTOR_AGENT_RECORDER_MODE:-normal}

case "${agent_recorder_mode}" in
  normal | version-mismatch | malformed-capabilities | passive-unavailable | non-json-report) ;;
  *)
    echo "MCP_DOCTOR_AGENT_RECORDER_MODE is unsupported" >&2
    exit 70
    ;;
esac
...
case "${1:-}" in
  --version)
    ...
    if [[ "${agent_recorder_mode}" == version-mismatch ]]; then
      printf 'mcp-doctor 9.9.9-eval\n'
    else
      printf 'mcp-doctor 0.4.1\n'
    fi
    ;;
  capabilities)
    ...
    if [[ "${agent_recorder_mode}" == malformed-capabilities ]]; then
      printf '{malformed synthetic capabilities\n'
    elif [[ "${agent_recorder_mode}" == passive-unavailable ]]; then
      agent_recorder_emit_capabilities | sed \
        's/"activity":"passive"/"activity":"active"/'
    else
      agent_recorder_emit_capabilities
    fi
    ;;
  inspect)
    ...
    if [[ "${agent_recorder_mode}" == non-json-report ]]; then
      printf 'synthetic non-JSON diagnostic\n'
      exit 1
    fi
    agent_recorder_emit_report
    exit 1
