...
| Product state | The passive local STDIO MVP, pinned current-revision compatibility matrix, bounded local and Streamable HTTP `check`, deterministic `break`, passive legacy inspection, current-revision contract snapshots and diffs, one-run JSON/JUnit report artifacts, conservative offline aggregation, structured unsupported-version diagnosis, and stable schema-backed JSON and JUnit-compatible projections pass local, protected hosted, immutable-release, and represented installed-channel evidence in `v0.3.0`; optional compiled-only capability discovery and explicit passive legacy-revision snapshot/diff support pass local and protected hosted source evidence without target replay, a new release, or a broad compatibility claim; completed `MCPD-027` and `MCPD-028` source support adds exact-selected MCP `2025-11-25` and `2025-06-18` `check` and `break`, with the latter requiring an exact Draft 2020-12 declaration before active schema use; both pass local synthetic, clean package/install, protected exact-head, and exact-`main` native CI, CodeQL, release-preflight, and retained compatibility evidence, while no MCP `2025-06-18` real-server, published active, or ecosystem-wide compatibility claim follows |
| Current milestone | M4 — enterprise assurance and adoption; `MCPD-016A` is Done and `MCPD-017` is In progress |
| Overall status | M0 through M3 are Done; immutable `v0.1.0`, `v0.2.0`, and `v0.3.0` channels are verified; protected `main` retains the `MCPD-013` controls; the scoped `MCPD-014` repository-security baseline and revalidated three-repository `MCPD-015` public community, channel, and license contract pass with their explicit limitations; dated `MCPD-016` evidence and the completed `MCPD-016A` correction prove the current supply-chain boundary, including direct digest-authenticated Syft acquisition, the narrowed live Action allowlist, and first-attempt exact-head and exact-`main` gates; `DEC-041` and `DEC-042` fix the organization-access policy; owner-authorized `MCPD-017` activation has applied the supported member, App, OAuth, PAT, short-lived verifier, secure-2FA, credential-inventory, deploy-key, and private-recovery controls, with closure still withheld until the protected change merges and the non-disclosing verifier passes on exact `main`; optional `MCPD-021` through `MCPD-028` are Done for resolved issues #72, #73, #64, #66, #74, #60, and #61 plus the independently verified `v0.3.0` release without changing the M4 gate |
| Current focus | Complete the `MCPD-017` exact-`main` closure independently; completed optional `MCPD-019` through `MCPD-028` and active optional `MCPD-029` remain outside the M4 dependency chain and cannot delay, redefine, or become prerequisites for `MCPD-017` or `MCPD-018` |
...
source scope. `MCPD-029` is active optional work for issue #75 under `DEC-053`.
`MCPD-027` established the shared active legacy boundary that `MCPD-028`
consumed; `MCPD-029` has no semantic dependency on legacy
...
None is part of M4. The merged `MCPD-027` and `MCPD-028` source changes the
exact MCP `2025-11-25` and `2025-06-18` active-command and capability-manifest
boundaries under their scoped evidence, but no published artifact or
ecosystem-wide compatibility claim follows. The active ticket changes no
merged or published active-command claim before its own acceptance evidence
passes; its working-tree implementation remains explicitly In progress.
...
| D-20 | Bounded schema-invalid tool-argument rejection diagnosis | Optional active correctness | In progress | `DEC-053` resolves `OPEN-16`; exact implementation head [`b33f3fd`](https://github.com/EnjoyableWork/mcp-doctor/commit/b33f3fd2a4fa4f34d953da632a8e599792610b8d) on draft [PR 82](https://github.com/EnjoyableWork/mcp-doctor/pull/82) implements the current-only `reject` surface and focused synthetic evidence for [issue #75](https://github.com/EnjoyableWork/mcp-doctor/issues/75). The complete local and dependency gates, two byte-identical dirty-tree package runs, packaged POSIX install smoke, release-binary POSIX smoke, PowerShell parser, and exact-head [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820998423), [CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820996753), and [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820998413), including native Windows and represented installed-channel evidence, pass; readiness, protected squash merge, issue closure, and durable exact-`main` evidence remain open and no published artifact claim follows |
...
| MCPD-029 | Diagnose bounded schema-invalid tool-argument rejection for the current active revision | Optional active correctness | In progress | `MCPD-012` | Under `DEC-053`, exact implementation head [`b33f3fd`](https://github.com/EnjoyableWork/mcp-doctor/commit/b33f3fd2a4fa4f34d953da632a8e599792610b8d) on draft [PR 82](https://github.com/EnjoyableWork/mcp-doctor/pull/82) implements [issue #75](https://github.com/EnjoyableWork/mcp-doctor/issues/75) through one exact independently authorized tool and effect boundary, seven deterministic mutation kinds each transmitted only after one proven structural mismatch, exact current-revision `-32602` rejection without prose matching, distinct unsafe success, malformed response, wrong identifier, disconnect, transport, timeout, crash, and cleanup outcomes, value-free reproduction, causal reporter parity, capability inventory, README/CLI coverage, and represented installed smokes through the settled active adapter. Formatting, Clippy, the full locked all-target/all-feature test suite, `cargo deny`, two byte-identical dirty-tree package runs, packaged POSIX install, release-binary POSIX smoke, PowerShell parser, and exact-head [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820998423), [CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820996753), and [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820998413), including native Windows and represented installed-channel evidence, pass; readiness, protected squash merge, issue closure, and durable exact-`main` evidence remain open |
...
| RISK-26 | A schema-invalid rejection diagnostic executes a side effect, mistakes success for safe rejection, or leaks the deliberately invalid request or server response | Critical | `DEC-053` and `MCPD-029` require the existing exact tool, effect, side-effect, target, and credential gates; prove each finite case invalid for one fixed structural reason before transmission; treat only exact structural `-32602` rejection as the expectation rather than safety authority; stop on unsafe success or exhausted safety bounds; and retain only value-free reproduction and causal evidence | Open while `MCPD-029` is In progress; exact implementation head [`b33f3fd`](https://github.com/EnjoyableWork/mcp-doctor/commit/b33f3fd2a4fa4f34d953da632a8e599792610b8d), focused synthetic evidence, the full local and dependency gates, and exact-head [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820998423), [CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820996753), and [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31820998413) exercise the accepted boundary on draft [PR 82](https://github.com/EnjoyableWork/mcp-doctor/pull/82), but it remains unmerged and unpublished until readiness, protected merge, issue closure, and exact-`main` evidence pass |
