...
The audit assigned legacy release-only fixed polling, broad curl retry, and
source-checkout runner verification to `MCPD-031`. That ticket is Done as the
prepublication prerequisite for another public version; it changes no product
runtime or immutable historical release.
...
after first-attempt exact-head [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31971256506),
[CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31971255580),
and [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31971256460)
passed. The first
...
`v0.3.0`. The failed attempt is preserved rather than rerun. Focused
[PR 91](https://github.com/EnjoyableWork/mcp-doctor/pull/91) made one explicitly
selected current stable version govern every rehearsed public handoff, passed
first-attempt exact-head [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972217149),
[CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972215668),
and [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972217150),
...
The corrected source then passed first-attempt exact-`main`
[CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972737294),
...
and first-attempt exact-head [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31844464763),
[CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31844463226), and
[release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31844464938)
prove one invocation-local choice between the existing `default` diagnostic
limits and a finite `slow-start` profile for `inspect`, `check`, and `break`,
without numeric overrides, configuration, retry, target authority, a
dependency, or release claims. It did not delay, redefine, or become a
prerequisite for `MCPD-017` or `MCPD-018`.

`MCPD-033` completed optional active-correctness work for GitHub issue #71 under
`DEC-056`. The separately versioned, current-revision workflow scenario now
supports a finite reviewed sequence of exact tools while preserving redundant
authorization, structural-only data flow, sequential execution, causal skips,
redaction, and independent cleanup evidence. It did not delay, redefine, or
become a prerequisite for `MCPD-017` or `MCPD-018`.

`MCPD-034` completed exactly two confirmed native-identity corrections in the
coordinated `v0.3.1` release under `DEC-058`: authority-bearing local inputs
and report artifact publication. Both advisories are public after represented
installed-channel verification. The quadratic SSE and schema-evaluation drafts
remain separate work and did not delay or broaden this patch.

`MCPD-035` implemented and published the optional adoption-UX slice in
immutable `v0.3.2`. It packages the existing CLI and stable-report behavior as
one concise vendor-neutral Agent Skill, one immutable versioned companion
artifact, and one canonical coding-agent guide. Every represented installed CLI
exposes one terse help route to the explicit, reversible skill install; no
package channel silently modifies an agent host. Exact Codex CLI `0.147.0`
discovery and explicit synthetic observations support that one scoped route.
Claude Code, Cursor, VS Code/Copilot, Kiro IDE, Kiro CLI, and Kiro Crew remain
unclaimed where their first exact observation was variant, unavailable, or
incomplete; scheduled, proactive, webhook, heartbeat, App, synthesized-skill,
and other unattended Crew behavior also remains unclaimed. Protected PR 96
advanced the `0.3.2` release-rehearsal default; exact protected `main` commit
`9f3a838` passed first-attempt CI, CodeQL, and release preflight plus the
defaulted release/OIDC, wrong-workflow rejection, tap no-write, and ten-job
channel rehearsals with clean before-and-after credential inventories.
`MCPD-035` is Done. The ticket adds no product execution authority, direct
...
`v0.3.3` patch-release candidate while retaining separate
implementation, regression, affected-line, and disclosure evidence for each
advisory. Its immutable GitHub Release is now public, but `DEC-064` preserves
the first release run's attestation-readiness failure before crates.io and
requires a new reviewed recovery path plus every represented downstream gate
before either advisory changes state.
...
| M4 | The `DEC-034`-locked OSPS `v2026.02.19` Level 1 controls pass with dated public evidence and official self-assessment proof; every canonical M3 GitHub Release asset is separately evaluated against SLSA `v1.2` Build L2, or a superseding decision explicitly replaces the target before proof | Done |
...
| `MCP-SAFETY-003` | Critical | An explicitly declared workflow cleanup step fails |
...
| `MCP-SCENARIO-001` | Error | A scenario violates its selected strict versioned contract |
...
| `MCP-WORKFLOW-001` | Error | A declared structural capture is absent from validated `structuredContent` |
...
| Schema and instance | Schema 1 MiB; instance 1 MiB; 100,000 schema nodes; depth 64; local `$ref` depth 32; 100,000 metered traversal, meta-validation, construction, reference, string, pattern-by-text, collection, combinator, equality, uniqueness, and instance-evaluation operations; 100 collected validation errors; linear-time pattern syntax only; 100,000-byte compiled-regex and DFA-cache limits |
...
disabled, a rejecting retriever, one shared operation budget, and its
linear-time pattern engine under fixed compiled-size and DFA-cache limits; the
locked normal dependency graph contains no HTTP or TLS client.
Backtracking-only pattern syntax receives the typed
`unsupported_linear_pattern` rule rather than being run by the optional
backtracking engine. Draft 2020-12 meta-schema errors and compilation errors
...
version. This release was completed independently of `MCPD-017` and does not
convert its now-completed private organization evidence into a public claim.
...
| D-09 | Evidence-backed enterprise assurance baseline | M4 | Done | The owner-published [BadgeApp baseline-1 record](https://www.bestpractices.dev/en/projects/14096/baseline-1), [24-control OSPS crosswalk](docs/assurance/osps-v2026.02.19-level-1.md), [seven-asset SLSA evaluation](docs/assurance/slsa-v1.2-build-l2.md), canonical [assurance projection](.github/assurance-controls.json), and bounded [public verifier](scripts/verify-assurance-evidence.sh) prove the scoped result and its maintenance/removal contract |
...
| D-22 | Deterministic release-state, retry, and runner-tool correction | Optional release maintenance | Done | `MCPD-031`, protected [PR 90](https://github.com/EnjoyableWork/mcp-doctor/pull/90), focused correction [PR 91](https://github.com/EnjoyableWork/mcp-doctor/pull/91), complete local and first-attempt exact-head gates, corrected first-attempt exact-`main` gates, and the protected nonpublishing release/OIDC rehearsal prove bounded direct publication-state observation, exact runner verification, and removal of broad release retries without changing immutable `v0.1.0`, `v0.2.0`, or `v0.3.0` bytes |
...
| D-26 | Bounded SSE decoding and actual JSON Schema work | Security correction | In progress | `DEC-060`, `DEC-061`, `DEC-062`, `DEC-063`, `DEC-064`, and `MCPD-036`; local verification confirms the fragmented-SSE regression in `v0.3.0` through `v0.3.2` and the schema-work accounting gap in `v0.1.0` through `v0.3.2`. The immutable `v0.3.3` GitHub Release exists, but its first workflow failed at asynchronous release-attestation availability before downstream publication. Completion requires the new reviewed exact-byte recovery, corrected operator audit, fail-closed before-and-after credential inventories and nonpublishing rehearsals on that source, crates.io and tap handoffs, represented installed-channel verification, and coordinated publication of both corrected advisories. |
...
| MCPD-017 | Establish organization access, credential, ownership, and recovery policy | M4 | Done | `MCPD-016A` | `DEC-041`, `DEC-042`, protected [activation PR 48](https://github.com/EnjoyableWork/mcp-doctor/pull/48), the verified [canonical projection](.github/organization-controls.json), [authenticated verifier](scripts/verify-organization-controls.sh), and [credential-free negative rehearsal](scripts/rehearse-organization-controls.sh) fix and test the selected boundary; exact `main` [`18c4495`](https://github.com/EnjoyableWork/mcp-doctor/commit/18c449530a5be0896c799d950c6c13085e25f785) passed first-attempt CI, CodeQL, release preflight, the dated non-disclosing live verifier, owner-only App-installation readback, private recovery proof, and confirmed one-run credential revocation without publishing identities, App mappings, recovery material, or credentials |
| MCPD-018 | Self-assess, publish, and maintain the enterprise assurance baseline | M4 | Done | `MCPD-017` | Exact targets were revalidated on 2026-08-15; all 24 OSPS Level 1 rows pass in the dated crosswalk and owner-published [BadgeApp project 14096](https://www.bestpractices.dev/en/projects/14096/baseline-1); all seven canonical `v0.3.0` assets pass the scoped SLSA Build L2 evaluation by exact digest and constrained signed provenance; the protected closure and exact-`main` public verifier pass without changing release bytes; annual and event-driven correction/removal triggers remain active |
...
| MCPD-031 | Replace legacy release polling, broad network retries, and implicit runner tools before the next public version | Optional release maintenance | Done | `MCPD-030`, `MCPD-024` | Protected [PR 90](https://github.com/EnjoyableWork/mcp-doctor/pull/90), exact implementation head [`eede359`](https://github.com/EnjoyableWork/mcp-doctor/commit/eede359edf9a0ee553ae0e6ffe284d121679531a), and merge [`4aaf3a4`](https://github.com/EnjoyableWork/mcp-doctor/commit/4aaf3a4180c97f8a6b55c90ec3d1f4d82877428f) remove broad release retries, fixed publication sleeps, and implicit source-checkout tools; focused [PR 91](https://github.com/EnjoyableWork/mcp-doctor/pull/91), exact correction head [`89ae1d0`](https://github.com/EnjoyableWork/mcp-doctor/commit/89ae1d02477b701ffffbf83315880757580d47e7), and merge [`f897049`](https://github.com/EnjoyableWork/mcp-doctor/commit/f8970496e416d85f17a018a3bcae262518418581) bind the rehearsal to the explicitly selected stable version represented by every public handoff. Complete local gates, actionlint, policy regressions, byte-identical packages and handoffs, both first-attempt exact-head CI/CodeQL/preflight sets, preserved deterministic failed [run 31971756990](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31971756990), and corrected first-attempt exact-`main` [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972737294), [CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972737006), [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972737323), and protected nonpublishing [release/OIDC rehearsal](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31972748664) pass without retrying publication, integrity, a test, job, or workflow or changing an immutable artifact, product timeout, live setting, M4 gate, or product runtime. |
| MCPD-032 | Add invocation-local named bounded diagnostic limit profiles | Optional runtime ergonomics | Done | `MCPD-004`, `MCPD-010`, `MCPD-025` | `DEC-055`; protected [PR 85](https://github.com/EnjoyableWork/mcp-doctor/pull/85), exact implementation head [`565ece0`](https://github.com/EnjoyableWork/mcp-doctor/commit/565ece032b637374e770a092219f346de63238d9), and closed [issue #65](https://github.com/EnjoyableWork/mcp-doctor/issues/65) prove exact `default` and `slow-start` selections on `inspect`, `check`, and `break`. The typed runtime mapping changes only the five accepted patience fields, keeps cleanup at two seconds, and leaves every byte, message, schema, retry, redirect, concurrency, credential, network, process, side-effect, and tool-authority limit identical. Complete locked local gates, `cargo-deny`, source review, two byte-identical packages (`sha256:32b3de958e95e8b8a4400017e573f347f0c6241f22ad1c1ab9d1252df3ff59d2`), packaged install, represented POSIX and PowerShell smokes, deterministic STDIO/HTTP and active no-authorization journeys, pre-target CLI rejection, and human/JSON/JUnit/aggregate/capability parity pass. First-attempt exact-head [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31844464763), [CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31844463226), and [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31844464938) pass on 2026-08-14 without an individual override, configuration source, unbounded/adaptive mode, timing-based acceptance, retry, fallback, dependency, publication, compatibility, assurance, or M4 claim. |
...
| MCPD-036 | Correct fragmented-SSE and JSON Schema resource amplification in one coordinated patch candidate | Security correction | In progress | `MCPD-035`, `MCPD-010`, `MCPD-006` | `DEC-060`, `DEC-061`, `DEC-062`, `DEC-063`, and `DEC-064`; replace repeated full-buffer SSE parsing with one incremental decoder whose scan work is linear in accepted bytes and whose retained line, event, and payload state remains bounded; meter actual Draft 2020-12 meta-validation, compilation, and instance-evaluation work with a deterministic operation budget that stops expensive library work rather than relying on elapsed time; preserve external-reference rejection, protocol/revision behavior, request-scoped completion, typed limits, causal skips, redaction, no-call-on-local-limit, and cleanup. Prove fragmentation-invariant SSE outcomes for single, irregular, and one-byte chunks plus partial UTF-8 and CR/LF boundaries; prove deterministic exhaustion for schema-by-instance cross-products, nested combinators, local-reference fan-out, string/pattern work, and uniqueness while ordinary schemas retain behavior. Correct the private advisory ranges to SSE `>=0.3.0, <=0.3.2` and schema `>=0.1.0, <=0.3.2`; keep both private until one patched release and represented installed-channel verification pass. Direct `referencing =0.49.9`, `jsonschema-regex =0.49.9`, and `regex-syntax =0.8.11` declarations are permitted only if their recorded reviews prove that reuse of the already locked transitive packages adds no package, feature, source, build script, unsafe code, platform, or MSRV change. Preserve both failed stale-contract audits, the rejected fail-open credential inventory, and the sole `v0.3.3` attestation-readiness failure; authenticate historical Homebrew evidence only at its recorded full commit, keep rolling tap state under the existing repeat-release and current-channel gates, observe disabled auto-merge through the exact read-only GraphQL field without granting contents write, and fail closed when any required secret inventory is unavailable. Finish only after independent focused regressions, complete locked local and dependency gates, deterministic package/install evidence, protected exact-head and exact-`main` gates, a corrected first-attempt operator audit and credential inventories plus nonpublishing rehearsals on new recovery source, exact immutable-source crates.io recovery, tap and represented installed-channel handoffs, and coordinated advisory publication. |
...
#### MCPD-036 `referencing` direct-use review — 2026-08-17

The public `jsonschema` build API repeats meta-validation internally but does
not expose its crate-private skip control, while its public generic-instance
API permits the operation-metered representation required by `MCPD-036`.
`jsonschema` does not re-export a generic Draft 2020-12 meta-validator. Direct
use of `referencing` is therefore limited to its pinned embedded
`SPECIFICATIONS` registry and `meta::DRAFT202012` document so `mcp-doctor` can
perform the same meta-validation through the metered representation before
the pinned compiler runs. Reimplementing or copying the meta-schema and
registry would create a larger, divergent protocol and maintenance boundary.

The stable, non-yanked `0.49.9` crate was published on 2026-08-09 with crates.io
checksum
`6efa2154ea6f5ce0fdecdd2a8d18f2fa1a39a8fbba91564f555a592e4dce8278` from
the established `Stranger6667/jsonschema` repository. Its `rust-v0.49.9` tag
resolves to verified commit
`4ad172231ae487511bfc4cf3199d7edab328923b`; the same maintainer is the sole
current crates.io owner. The repository is active, unarchived, and received a
push on the review date. The release notes concern canonicalization rather
than the reference/meta-schema surface used here. The upstream has an active
issue, discussion, and release path but no dedicated `SECURITY.md`; that
weaker formal response route remains a replacement or re-review trigger and
is supplemented by the repository's existing RustSec and `cargo-deny`
monitoring.

The crate is MIT licensed, declares Rust `1.85.0` and edition 2021, contains
one library target, no build script, and no `unsafe` block in the reviewed
published source. Its default feature set is empty; the sole optional
`retrieve-async` feature and its async dependencies remain unselected. The
normal graph already contained this exact registry source, checksum, version,
platform dependency set, and empty default feature through
`jsonschema =0.49.9`. `Cargo.toml` and `Cargo.lock` add only the root dependency
edge: no package, duplicate, source, checksum, feature, build script, license,
platform, or effective MSRV changes. The direct edge itself adds no additional
build, binary, startup, or network path. Product use lazily constructs one
trusted embedded meta-validator during schema analysis and retains the
existing rejecting external retriever; deterministic schema-budget,
package/install, native, and dependency gates own its behavioral and cost
evidence. Any version, owner, source, feature, retrieval, meta-schema,
registry, platform, or broader API use requires the complete dependency
review again.

#### MCPD-036 pattern-analysis direct-use review — 2026-08-17

The standard linear-time regex engine bounds one search by the product of the
compiled expression and haystack sizes. Charging only source-pattern bytes and
instance bytes separately therefore leaves their product, including
counted-repetition expansion, outside the named schema-work counter.
`jsonschema` does not expose either its translated ECMA-262 pattern or compiled
automaton size. Direct use of `jsonschema-regex` and `regex-syntax` is limited
to the same public translation plus a read-only HIR traversal before the
validator runs. That maintained parser is safer and more faithful than a local
regex parser; the resulting finite structural cost is multiplied by all
potential instance text and charged before matching. The configured compiled
regex and DFA cache limits are additionally reduced to 100,000 bytes each.

`jsonschema-regex 0.49.9` is the stable, non-yanked companion published with
the reviewed 2026-08-09 `jsonschema` release. Its crates.io checksum is
`91994f45017ed5e66aa8e59b8415f4cb033a6380d7200387b7cf117595fbdf85`;
the same signed `rust-v0.49.9` tag resolves to verified commit
`4ad172231ae487511bfc4cf3199d7edab328923b` in the active, unarchived
`Stranger6667/jsonschema` repository; Dmitry Dygalo remains its sole current
crates.io owner. The package is MIT licensed, declares Rust `1.85.0` and
edition 2021, contains one library target, no build script, no feature table,
and no `unsafe` block in the reviewed published source. Its only normal
dependency is the already locked `regex-syntax` package.

`regex-syntax 0.8.11` is a stable, non-yanked 2026-06-09 release from the
active, unarchived Rust project `rust-lang/regex` repository. Its crates.io
checksum is
`d6f6ff9a378485b298a5286656da665ba74413d36db0979633275d2e708145d4`;
the `regex-syntax-0.8.11` tag resolves to signed verified commit
`140167995737fa11dfe11b8af8b9aa143b790b4e`. The repository has an explicit
Rust Security Response WG route and active issue, pull-request, discussion,
and release paths; current crates.io ownership includes Andrew Gallant, the
Rust project owner, and the regex-owners team rather than one individual
publisher. The package is MIT/Apache-2.0 with Unicode-licensed tables, declares
Rust `1.65` and edition 2021, forbids unsafe code, has no build script or normal
dependency, and supports every represented target. Direct defaults are
disabled; naming `std` and `unicode` preserves the feature set already selected
by the locked validator graph while keeping the unrelated optional `arbitrary`
dependency absent.

Both exact registry packages, checksums, sources, platform code, licenses, and
effective features were already in `Cargo.lock`. The manifest and lockfile add
only two root edges: no package, duplicate, source, checksum, feature, build
script, native code, platform, network path, or effective MSRV changes. The
new product work translates and parses each bounded schema pattern once,
caches only its value-free structural cost during compilation, and performs no
I/O. Deterministic pattern expansion, pattern/property cross-product,
ordinary-semantics, no-call, package/install, native, and dependency gates own
its behavioral and cost evidence. Any version, owner, source, feature, parser,
translation, regex-engine, cost-model, or broader API use requires complete
re-review.

#### MCPD-036 local candidate evidence — 2026-08-17

The local implementation gate passes `scripts/check.sh`, including formatting,
Clippy with warnings denied, 122 unit tests, and every built-binary and
integration suite. `cargo deny --all-features --locked check`,
`scripts/verify-source-artifacts.sh --worktree`, `git diff --check`, the
local supply-chain inventory policy tests, and manifest/lockfile metadata
checks also pass. ShellCheck `0.11.0`, Actionlint `1.7.12`, Bash syntax checks,
and JSON parsing pass for the changed automation and policy files. The
authenticated exact-source supply-chain verifier is not
local passing evidence: the current session cannot read its required
administrator-only Actions and secret-inventory fields, and the changed tree
is not clean exact `main`; its passing source identity remains a post-merge
gate. The focused evidence includes fragmentation-invariant and linear-work
SSE decoding; bounded retained decoder state; schema combinator, reference,
collection, equality, uniqueness, pattern-expansion, pattern/property
cross-product, and translation work; the shared compiled-schema plus instance
counter; independent generated-input aggregate arithmetic; and active human,
JSON, and JUnit no-call/redaction behavior when the schema budget is exhausted.
The injected work marker preserves a valid source-owned empty-keyword schema
target, and a differential ordinary-schema suite matches the pinned Draft
2020-12 validator across booleans, local references, applicators, arrays,
dependencies, patterns, equality, and unevaluated properties.

The first complete gate after tightening the shared counter failed
deterministically because a built-binary fixture still expected the later
`active_input_bytes` boundary for a generated string whose combined compile and
evaluation work now correctly reaches `schema_evaluation_steps` first. No
unchanged source was rerun. The correction preserves that earlier no-call
diagnosis in the built-binary journey and proves the separable aggregate-input
arithmetic directly at the unit layer; the next complete gate passed.

The clean implementation commit immediately preceding this evidence-only
update passed two successive `cargo package --locked` runs that produced
byte-identical 182-file `0.3.3` source packages with SHA-256
`7fa45ca89e3861b1b31ea727994d9158c5279113075c9adc9870963e9f5fa7dc`.
Two `scripts/generate-release-channels.sh` runs produced identical Cargo and
Homebrew source inputs. Installing that exact extracted package through the
locked path, building its exact synthetic fixture, and running the full
`scripts/smoke-installed.sh` journey passed. The manifest, lockfile, release
notes, support guidance, issue form, and Agent Skill consistently identify the
unpublished `0.3.3` candidate. The canonical `0.3.3` Agent Skill source verifies
at SHA-256
`4ef5796bded1d2b7261e1b7d330c347aa9dfde9f7826cb8ab879290d9a40b1cf`.
These disposable clean exact-source outputs are candidate evidence, not
immutable release evidence: the release workflow rehearsal default
intentionally remains the latest public `0.3.2`, the macOS host lacks the GNU
tar required for deterministic release and Agent Skill archives, and
...
server-generated release attestation for the tag object. The crates.io job was
skipped; `0.3.3` remains absent there, the tap remains at `0.3.2`, no installed
channel verifier was dispatched, and both advisories remain private.
...
`O_NOFOLLOW` or `O_NONBLOCK` constants. Copying target-specific numeric values
would create an unreviewable portability boundary, while a larger filesystem
crate would add implementation and transitive surface. `libc` `0.2.189` is
therefore selected only for those two constants; Windows retains the existing
focused standard-library plus `GetFileInformationByHandleEx` implementation.

The stable non-yanked `0.2.189` release was published on 2026-07-21 from the
annotated `0.2.189` tag in the established `rust-lang/libc` repository. The
repository is active and unarchived under the Rust project, crates.io ownership
is limited to the Rust project owner and libs team, and the Rust Security
Response WG provides the organization security-response path. The crate is
MIT/Apache-2.0, declares Rust `1.65`, supports every represented target, and
contains raw platform declarations plus a build script. Those surfaces were
already present and executed through the exact locked runtime graph before
direct selection.

Defaults remain disabled. Cargo metadata and the lockfile show no new package,
version, feature, source, duplicate, or checksum; the root dependency edge is
the complete graph diff. Product code uses no `libc` function, type, pointer,
allocation, or unsafe block—only `O_NOFOLLOW | O_NONBLOCK` when constructing a
read-only Unix `OpenOptions`. The release binary gains no separately linked
library, startup work, or runtime work beyond the required kernel open flags.
Focused forced-replacement, symlink, disappearance, hard-link, scenario,
custom-trust, snapshot, aggregate, and native checks own the behavioral
evidence. Any version, ownership, source, build-script, constant, platform, or
use expansion requires the complete dependency review again.
...
| MCPD-032 | Resolve GitHub issue #65 without delaying M4: add one invocation-local `--limit-profile default|slow-start` selection to `inspect`, `check`, and `break`; keep `default` runtime behavior and numeric limits compatible while adding its explicit name and effective values to report metadata; let `slow-start` increase only startup, discovery, request, response, and total patience to the exact finite values in `DEC-055`; and preserve the two-second cleanup bound plus every byte, count, retry, redirect, concurrency, target, process, network, credential, side-effect, protocol, and tool-authorization boundary. Reject unknown names before target preparation, retain the selected profile and complete effective values in stable reports and offline aggregates, inventory selection support in compiled capabilities, and add deterministic cross-command, transport, artifact, rejection, and represented installed-source evidence without timing-based acceptance. Add no per-field override, environment or file configuration, unbounded/adaptive mode, fallback, dependency, release, or M4 claim. Finish only when complete local and first-attempt exact-head hosted gates pass, the protected change merges, issue #65 closes, and durable completion evidence is recorded. |
| MCPD-033 | Resolve GitHub issue #71 without delaying M4: define and implement the `DEC-056` current-revision `mcp-doctor.scenario/v2alpha1` workflow as the smallest safe multi-tool vertical slice. Reuse one active adapter and both existing transports; require exact step tools, per-step effects, redundant invocation authorization, structural captures and prior-only references, one sequential main path, and one explicit contiguous cleanup suffix. Preserve all process/network/credential/schema/time/byte/report/redaction gates, v1 behavior, stable report compatibility, and cleanup prominence. Add deterministic positive read-only and side-effect-plus-cleanup journeys and negative authorization, reference, pointer, capture, schema, response, timeout, disconnect, early-stop, cleanup, limit, reporter, artifact, aggregate, installed-platform, and legacy-rejection evidence. Add no loops, branches, expressions, interpolation, arbitrary environment access, dynamic tool selection, LLM planning, retry, concurrency, dependency, release, broad compatibility, assurance, or M4 claim. Finish only when the complete local and first-attempt exact-head hosted gates pass, the protected change merges, issue #71 closes, and durable completion evidence is recorded. |
| MCPD-034 | Correct the two related native-identity advisories without public pre-disclosure. For authority-bearing inputs, replace every path-precheck/following-open sequence for scenarios, custom CA trust, snapshots, and aggregate reports with one focused no-follow native-identity opener; make opened-handle metadata authoritative; verify the selected path still identifies that handle before reading; and fail closed without target start, network connection, credential resolution or transmission, or accepted offline evidence. For report artifacts, record the complete identity of every opened stage, require the stage path to match immediately before a no-overwrite link, require the new destination to match immediately afterward, and clean up or roll back only paths that still identify owned files. Preserve every byte, certificate, schema, authorization, redaction, all-or-nothing publication, and reporter contract; cover JSON, JUnit, two-output rollback, aggregate output, replacement, disappearance, symlink/reparse, hard-link, no-foreign-delete, and Unix/Windows identity through deterministic event/acknowledgement interleavings; add no retry, overwrite authority, raw path/value output, or unreviewed dependency; exclude the SSE and schema-evaluation drafts from `v0.3.1`; and finish only after private exact-head verification, one coordinated patched release, both advisory publications, exact-`main` gates, and a non-sensitive durable completion record. |
| MCPD-035 | Begin only after `MCPD-034` is Done. Follow `DEC-057` as superseded by `DEC-059`. Create one repository-owned canonical `.agents/skills/mcp-doctor/SKILL.md` using only the portable required `name` and `description` frontmatter, keep all behavioral instructions in that file, and bundle no executable or permission-bearing resource. Bind those exact bytes to one immutable versioned companion archive and documented CLI compatibility identity in the next protected patch release. Add one terse top-level `mcp-doctor --help` route to the exact release-bound HTTPS rendering of `docs/agents.md` and a README “Use with coding agents” route that state the current interaction is coding agent → terminal → `mcp-doctor` CLI → target MCP server, not an `mcp-doctor` MCP server configuration. Host-specific material may only install those canonical bytes through a primary-documented explicit reversible route, verify discovery, and remove them; never silently mutate a host from Cargo, Homebrew, package installation, startup, or another channel, pipe an unpinned remote installer into a shell, overwrite an existing skill, or treat CLI installation as skill installation. Make the instruction-only skill check `mcp-doctor --version` and `mcp-doctor capabilities --format json`, stop with installation guidance rather than install software when the executable is absent, default to `inspect --format json`, require an exact user- or repository-selected command or endpoint, and interpret primary diagnosis, independent findings, causal skips, outcome, and exit without exposing raw values. Permit a code correction and rerun of the same passive inspection only when editing was requested. Never infer a target or escalate from discovery to `check`, `break`, or `reject`; declare no `allowed-tools` or equivalent permission grant; and never solicit, retain, or render secrets. Include a report-only CI handoff that reads an existing stable JSON artifact without rerunning the target. Add deterministic standard-format, canonical-byte/archive-digest, forbidden-field/permission/script, command-contract, documentation-link, help-route, install/removal, and redaction checks. Use a synthetic PATH-preferred `mcp-doctor` command recorder and disposable repository, target, report, user profile, and host state to prove the exact permitted command sequence and absence of real target, network, credential, side-effect, or publication activity for passive diagnose-only, explicitly requested fix-and-rerun, existing-report triage, missing-binary, ambiguous-target refusal, unauthorized-active refusal, and secret-bait prompts. Independently exercise the CLI without the skill to prove its existing active target, network, credential, tool, effect, side-effect, cleanup, and reporter gates remain authoritative. Revalidate primary vendor documentation at implementation time and record the exact product version, date, install and removal route, discovery path, skill-list result, explicit invocation result, and separately labeled implicit-selection observation for Codex, Claude Code, Cursor, VS Code/Copilot, Kiro IDE, Kiro CLI, and Kiro Crew. A clean-profile explicit invocation pass is required for each support claim; implicit selection is best-effort dated integration evidence and never a guarantee. Limit the Kiro Crew route to one foreground interactive session in a disposable data home, verify the canonical skill digest remains unchanged, and claim no schedule, proactive loop, webhook, heartbeat, App, synthesized skill, persistent lesson, background task, unattended execution, or cross-surface behavior. Treat every forward-test as a scoped integration observation rather than deterministic model-correctness or adoption proof; a failed or variant observation blocks that host claim instead of being rerun into acceptance. Update the opt-in adoption-evidence route to record exact host and workflow without telemetry and without treating downloads, binary or skill installs, stars, vendor compatibility, or forward-tests as independent usage. Finish only when the complete locked local gate, deterministic skill/distribution/document checks, protected exact-head checks, immutable next-patch skill and binary publication, represented installed-channel help verification, dated exact-version host matrix, and durable scoped evidence pass. Add no active authority, direct MCP mode, marketplace requirement, IDE extension, host plug-in, daemon, SDK, hosted service, dependency, silent or automatic installation, universal-agent claim, broad compatibility claim, or M4 reopening. |
| MCPD-036 | Correct both confirmed private CWE-400 reports without public pre-disclosure and prepare one coordinated patch release. For request-scoped SSE, replace full-buffer reparsing with a stateful incremental UTF-8 decoder that consumes every accepted byte once, handles CR, LF, CRLF, split code points, comments, priming events, notifications, server requests, EOF, and early matching responses, and retains only bounded current-line, current-event, and payload state. For Draft 2020-12 schemas, make the existing `schema_evaluation_steps` boundary meter the preliminary walk plus actual meta-validation, validator construction, instance access, string/pattern/equality work, collection traversal, combinators, local-reference fan-out, and uniqueness; stop synchronously with the existing typed limit and no target call when the budget is exhausted. Use the pinned validator's public custom-representation and custom-keyword boundaries rather than a timeout, background thread, subprocess leak, external retrieval, or casual replacement validator; use the already locked `referencing =0.49.9`, `jsonschema-regex =0.49.9`, and `regex-syntax =0.8.11` packages directly only after their complete dated reviews, including maintained HIR accounting for the pattern-by-text product before matching. Preserve every protocol revision, transport gate, authorization, message/byte/depth/reference/error limit, report schema, primary diagnosis, causal skip, redaction, cleanup, zero retry, and zero fallback contract. Add deterministic operation-count regressions for single/irregular/one-byte SSE fragmentation, partial UTF-8 and CR/LF splits, comment and priming compaction, oversized events, message exhaustion, nested `allOf`/`anyOf`/`oneOf`, `items` cross-products, local-reference fan-out, patterns, equality, uniqueness, meta-validation, compilation, passive and active no-call behavior, and human/JSON/JUnit parity; never use elapsed time as acceptance. Correct the SSE affected line to `>=0.3.0, <=0.3.2` and the schema line to `>=0.1.0, <=0.3.2`, set the patched line only after immutable publication, and keep both drafts private until complete local, dependency, package/install, protected exact-head, exact-`main`, release, and represented installed-channel gates pass. Publish both advisories only after those gates and retain separate causal, regression, and version evidence even though the release is coordinated. |
...
Operator isolation is an explicit maintenance boundary. A future reviewed
verifier token must be passed only as an invocation-local `GH_TOKEN`; do not
use `gh auth login` or treat an alternate `GH_CONFIG_DIR` as keychain isolation.
GitHub CLI can use host-wide macOS keychain storage despite a separate config
directory. Clear the value immediately after the one run, verify that the
ordinary CLI identity remains usable, revoke the token, and prove revocation
with a rejected API request.
...
had later been manually re-enabled for repository administrators. The
organization audit record identified an interactive browser settings change,
not an App action or automation identity. A read-only audit of all 18 current
workflows across `mcp-doctor`, `mcp-sync`, and `homebrew-tap` found no App
installation, repository-selection, organization-setting, or App-token
generation path; sharing `homebrew-tap` therefore did not cause the drift. The
owner restored the restriction, and a fresh UI readback confirmed that
repository administrators can no longer install GitHub Apps. Any later
settings or workflow change remains a review trigger.
...
negative readback proves that it receives `403` from the organization. The final
verifier credential was owned by `EnjoyableWork`, expired within 30 days, had
the exact canonical read-only permission profile, reached only the source and
tap, succeeded across the required organization and repository APIs, and
received `404` for an unrelated private repository. Its replacement value was
used once for the exact-`main` run, permanently deleted immediately after the
pass, and confirmed unusable by a rejected API request. The operator's ordinary
OAuth-backed GitHub CLI session was restored and rechecked after the temporary
login exposed the host-wide keychain behavior documented above.
...
The canonical lifecycle is verified with the reviewed aggregate App count and
recovery date, and the mode-private attestation was complete for the final run.
The live API also exposed localized fractional App timestamps, so the verifier
requests UTC explicitly, canonicalizes only bounded valid UTC timestamps, and
rejects a non-UTC negative. `MCPD-017` is Done. Exact `main`
[`18c4495`](https://github.com/EnjoyableWork/mcp-doctor/commit/18c449530a5be0896c799d950c6c13085e25f785)
passed first-attempt [CI](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31851562136),
[CodeQL](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31851561580),
and [release preflight](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31851562040).
After the corrected owner-only setting and private assertions were read back,
the 2026-08-15 live verifier emitted only
`date=2026-08-15 canonical_sha256=8f3b2c3db5f81a174d93bbcdaa8341e816b15c6ae059329fe5d61925c91a8111 source_sha=18c449530a5be0896c799d950c6c13085e25f785 result=PASS`.
The private attestation and API projections were then deleted with the revoked
credential. This accepted policy and completion evidence are not an achieved
...
### MCPD-018 completion evidence

The activation-locked targets were revalidated against their official sources
on 2026-08-15 before publication: OSPS remained at `v2026.02.19` with 24 Level
1 controls, BadgeApp still presented that exact baseline series, and SLSA
`v1.2` remained the approved specification. The owner created BadgeApp project
[`14096`](https://www.bestpractices.dev/en/projects/14096/baseline-1), reviewed
all proposed answers, and published the official-hosted self-assessment. Its
[public JSON](https://www.bestpractices.dev/projects/14096.json) reports 100%,
an achieved baseline-1 timestamp of 2026-08-15 22:14:15 UTC, 24 `Met` answers,
and 24 justifications linked to the dated [OSPS
crosswalk](docs/assurance/osps-v2026.02.19-level-1.md). The README embeds the
provider's badge linked back to that exact assessment.

The separate [SLSA `v1.2` Build L2
evaluation](docs/assurance/slsa-v1.2-build-l2.md) re-downloaded every one of the
seven canonical immutable `v0.3.0` GitHub Release assets, matched its recorded
byte count and SHA-256 digest, and used only the exact reviewed GitHub CLI
`2.97.0` host binary to verify constrained signed provenance for the repository,
signer workflow and digest, tag ref, source digest, GitHub OIDC issuer, hosted
runner, and SLSA provenance predicate. The bounded [assurance
verifier](scripts/verify-assurance-evidence.sh) repeats the public BadgeApp,
source, release-byte, and attestation checks and retains only structural,
non-sensitive output. After the protected closure and its exact-`main`
verification, `MCPD-018` is Done; D-09 and M4 are Done without changing an
immutable release byte.

These results are the scoped self-assessment and artifact evaluation described
by the two crosswalks. They are not independent certification, regulatory
compliance, higher OSPS levels, SLSA coverage of a channel operation,
dependency, unlisted asset, or future release, or paid platform signing.

### OSPS Level 1 control map

The locked OSPS baseline has 24 Level 1 controls. Existing files or settings did
not close a row by themselves: the owning tickets verified their effective
public or live state, and `MCPD-018` linked the resulting durable evidence.
...
| **Total** | **Every Level 1 control in the locked baseline** | **24** | The dated crosswalk and official BadgeApp record link evidence for every applicable control |
...
| DEC-056 | Resolve issue #71 with one finite structural workflow scenario and no general orchestration language | Accepted | 2026-08-14 | Add strict `mcp-doctor.scenario/v2alpha1` only to `check` for exact MCP `2026-07-28` over existing STDIO and Streamable HTTP. A document contains 1–100 ordered steps; every step has a unique review-only ID, exact tool, explicit effect classification, object arguments, and expected success or tool error. Optional `captures` map unique identifier-like names to RFC 6901 pointers relative only to a successful validated `structuredContent`; optional `argument_refs` map RFC 6901 destinations at existing null placeholders to captures declared by an earlier step. Permit at most 100 captures, one MiB per arguments object or captured value, and eight MiB each across active inputs and retained captures; release all captures before report construction. `target_env` and `secret_refs` retain the exact existing environment-only boundary and may not overlap reference destinations. The invocation repeats `--allow-tool` exactly once for every distinct declared tool, with no wildcard, duplicate, or extra, and separately passes `--allow-side-effects` when any step is side effecting; discovery, annotations, captures, and prior results never grant authority. Discover and locally validate every selected tool before the first call. Main steps run sequentially at concurrency one and the first failure, incomplete result, missing capture, unsafe response, transport loss, or exhausted limit causally skips later main activity. Only a contiguous suffix marked `cleanup: true`, expecting success, and declaring no new capture may continue after a main failure, and only while all referenced captures and safety budgets remain available; a failed cleanup stops later calls and remains an independent critical finding. Reports identify numeric main or cleanup step indexes plus fixed structural locations only and never retain author IDs, tool or capture names, pointers, arguments, results, endpoint, credential, environment source, or value. Keep accepted v1 documents, wire behavior, and successful report projection unchanged, reject legacy workflow selection before target preparation, and add no loop, branch, expression, interpolation, scripting, dynamic tool selection, LLM planning, automatic response, retry, concurrency, dependency, release, compatibility, assurance, or M4 claim. |
| DEC-057 | Use one vendor-neutral passive-first Agent Skill as the coding-agent adoption wedge | Superseded | 2026-08-16 | After `MCPD-034`, package the existing CLI and stable-report contract as one concise instruction-only `mcp-doctor` Agent Skill plus canonical user documentation. A coding agent invokes the CLI through its terminal rather than configuring `mcp-doctor` as an MCP server; host-specific guidance covers installation and discovery only and cannot fork the safety workflow. Begin with installed-version and compiled-capability checks, require an exact user- or repository-selected target, default to JSON passive inspection, preserve causal and independent report interpretation, and edit and rerun only when requested. Discovery never authorizes `check`, `break`, or `reject`; the skill grants no tools or permissions, installs no software, bundles no executable, and handles no secrets. Claim a Codex, Claude Code, Cursor, or VS Code/Copilot route only after dated primary-source and exact-version validation; treat forward-tests as scoped integration observations and independent opt-in reports as adoption evidence, never model-correctness, download, install, star, or market-dominance proof. Direct MCP mode, IDE extensions, plug-ins, daemons, SDKs, hosted services, and new runtime behavior remain outside this ticket. `DEC-059` retains this passive workflow and safety boundary while replacing its distribution, host set, no-CLI-change, and no-publication assumptions; the original decision remains visible rather than being rewritten. |
...
supply-chain, and corrective acquisition contracts. None of these decisions
constitutes the complete M4 assessment or converts `MCPD-017`'s scoped live
controls and private recovery evidence into a public assurance claim.
...
| RISK-15 | Organization-owner loss or over-broad long-lived credentials become an undocumented recovery dependency | High | `DEC-041` accepts the documented single-owner residual risk and fixes strong MFA, lowest access, application and credential scope, and private recovery requirements; `DEC-042` fails closed around GitHub Free's unavailable owner-only outside-invitation setting; `MCPD-017` binds the completed live controls, reviewed selected-repository App inventory, exact short-lived operator authority, and fresh private recovery exercise through protected exact-`main` evidence and confirmed credential revocation without sensitive disclosure | Mitigated for the completed `MCPD-017` scope: supported organization controls, restricted OAuth and PAT access, corrected owner-only App installation, exact source-and-tap verifier scope, bounded private inventories, obsolete tap-key removal, independent recovery, the dated exact-`main` pass, and rejected post-revocation API use all pass. The accepted single-owner availability residual remains; any owner, factor, membership, application, credential, recovery, workflow, platform-capability, or 90-day review trigger reopens this risk. |
| RISK-16 | A stale, unofficial, or over-broad assurance claim misleads adopters | High | `DEC-034` locks exact version and proof routes and makes drift a stop-and-decide gate; `MCPD-018` binds every claim to exact scope, date, official proof, public evidence, and removal triggers; missing, stale, withdrawn, or ambiguous proof blocks or removes the claim | Mitigated for the 2026-08-15 scope by exact-version revalidation, owner-published BadgeApp project `14096`, the dated 24-control OSPS and seven-asset SLSA crosswalks, bounded exact-`main` public verification, and explicit annual/event-driven correction or removal. Any framework, issuer, record, repository, organization, workflow, release, asset, digest, attestation, evidence, or claim-scope drift reopens this risk. |
...
| RISK-27 | Scheduler-dependent fixtures, correctness retries, or incidental runner tools make identical source pass and fail or hide a safety regression | High | `DEC-054`, `MCPD-030`, and `DEC-064` require a complete tracked timing/tool inventory; product bounds only in product code or explicit limit fixtures; audited product-clock input only for a contract that consumes verification time; job, socket, and channel deadlines only as outer watchdogs; observable state or event/acknowledgement fixture coordination; deterministic forced-state regressions; one exact immutable Syft acquisition exception under `DEC-043`; declared non-standard CI tools; preserved failed evidence; and first-attempt acceptance without rerun, timeout inflation, assertion weakening, quarantine, or broad serialization | Open for the release-only scope after the sole `v0.3.3` tag workflow verified and published exact immutable bytes but immediately failed because GitHub's asynchronous release attestation was not yet available. The later attestation diagnoses the race and is not a green rerun. `DEC-064` moves cryptographic release verification behind a separately approved observable-attestation boundary and permits only one new reviewed exact-byte recovery path after complete source, protected, operator, inventory, and rehearsal gates. Existing deterministic test and source-checkout evidence remains valid. Any unchanged rerun, timing threshold used as correctness proof, undeclared runner tool, inventory drift, broad retry, fixed publication sleep, mismatched handoff selector, or weaker cleanup/state-transition evidence keeps the risk open. |
