...
| Overall status | M0 through M3 are Done; immutable `v0.1.0` and `v0.2.0` channels remain verified; protected `main` retains the `MCPD-013` controls; the scoped `MCPD-014` repository-security baseline and revalidated three-repository `MCPD-015` public community, channel, and license contract pass with their explicit limitations; dated `MCPD-016` evidence remains historical, while exact-`main` release preflight exposed indirect mutable Syft acquisition and reopened `RISK-14` for focused `MCPD-016A` correction; `DEC-041` and `DEC-042` fix the organization-access policy; and staged owner-authorized activation has applied supported member and App-installation controls and removed the obsolete tap deploy key without claiming that App scope, credential policy and migration, private recovery, or the final verifier have passed |
| Current focus | Complete `MCPD-016A` by replacing indirect Syft acquisition with reviewed immutable Linux assets, bounded transient-only acquisition retries, deterministic offline negatives, and first-attempt exact-head and exact-`main` evidence; then resume the remaining `MCPD-017` App, credential, authority, inventory, and private-recovery activation |
...
| MCPD-016A | Remove indirect mutable Syft acquisition from release automation | M4 | In progress | `MCPD-016` | `DEC-043`; repository-owned Linux ARM64/x64 acquisition by immutable asset URL, exact size and SHA-256, reviewed archive layout, version and platform; transient-only bounded retry classification; deterministic offline negatives; removal of the obsolete Action and live allowlist entry; and first-attempt exact-head plus exact-`main` CI, CodeQL, and release-preflight evidence without changing an immutable release byte |
| MCPD-017 | Establish organization access, credential, ownership, and recovery policy | M4 | Blocked | `MCPD-016A` | `DEC-041`, `DEC-042`, the activation-only [canonical projection](.github/organization-controls.json), [authenticated verifier](scripts/verify-organization-controls.sh), and [credential-free negative rehearsal](scripts/rehearse-organization-controls.sh) fix and test the selected boundary; supported member privileges, owner-only App installation, and tap deploy-key removal are live, while selected App scope, PAT and OAuth policy, operator-authority migration, exact reviewed inventory, private recovery, and exact-main evidence remain completion work after `MCPD-016A` restores the required release-preflight boundary |
...
`MCPD-016A` remains In progress until the obsolete Action is absent from both
workflows and the live selected-Action allowlist; local gates pass; and the
protected change passes CI, CodeQL, and release preflight on the first workflow
attempt at exact head and exact `main`. The live allowlist narrows only after
the merged workflows no longer select the Action, preventing an ordering
window from blocking the current `main` workflow. The broader deterministic-CI
...
non-owner has direct repository-administrator authority. Any plan, native-field,
membership, invitation, outside-collaborator, or non-owner-administrator change
requires review. If GitHub makes the native restriction available, it must be
enabled and this compensating contract must be superseded.
...
The owner then explicitly authorized the staged live activation. API-supported
future-member repository, Pages, team, issue, visibility, deletion, and private
fork privileges are disabled; the GitHub App installation control is restricted
to owners; and the obsolete write-capable tap deploy key was deleted after its
job-scoped `GITHUB_TOKEN` replacement was revalidated. Readback reports zero tap
deploy keys. The remaining outside-collaborator invitation field is the GitHub
Free platform limitation governed by `DEC-042`, not an unrecorded successful
toggle.

Completion blockers remain. Four installed Apps still cover all repositories,
three have write permissions, and the exact selected-repository inventory has
not yet been owner-approved. App access requests are disabled rather than using
the accepted owner-review request flow. The current operator credential remains
a classic PAT. Secure-method enforcement, OAuth restriction, PAT policies, the
billing-manager aggregate, and the recovery operation still require private
confirmation or activation. One environment secret on the separately
classified public product and one variable on a private repository are outside
the repository-specific `mcp-doctor` claim; neither is treated as permission to
ignore any organization-wide reach.

The canonical lifecycle, App identity, and recovery date remain deliberately
incomplete, the live verifier fails closed, and `MCPD-017` remains temporarily
Blocked on `MCPD-016A` before its staged activation resumes.
The owner authorization covers the staged organization, App-restriction,
credential-migration, obsolete-key deletion, and private-recovery work, but it
does not turn incomplete or unverified state into evidence. This accepted policy
is not an achieved OSPS result, complete M4 baseline, certification,
regulatory-compliance claim, or assurance statement about a separate product.
