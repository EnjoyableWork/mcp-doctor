...
| Current focus | M4 remains complete with annual and event-driven claim review active; `MCPD-034` is Done, and proposed `MCPD-035` is now the next eligible optional coding-agent adoption-UX ticket |
...
`MCPD-035` is proposed optional adoption-UX work and is eligible now that
`MCPD-034` is Done. It packages the existing CLI and stable-report behavior as
one concise vendor-neutral Agent Skill and one canonical coding-agent guide;
host-specific material is limited to dated, tested installation and discovery
instructions. It adds no product execution authority, direct MCP-server mode,
IDE extension, daemon, SDK, hosted service, automatic installation, or release
prerequisite.
...
| D-25 | Safe portable coding-agent diagnostic workflow | Optional adoption UX | Proposed | `MCPD-035` requires one canonical instruction-only Agent Skill, `docs/agents.md`, a README route, a dated host-support matrix, deterministic structure and safety checks, passive and refusal forward-tests, and an opt-in adoption-evidence route before any coding-agent support or usage claim |
...
| MCPD-035 | Make the existing passive diagnostic workflow discoverable and safe for coding agents | Optional adoption UX | Proposed | `MCPD-034`, `MCPD-025` | Add one canonical vendor-neutral Agent Skill plus `docs/agents.md` and a README route. Prove its passive-first, exact-target, capability-aware, redacted-report interpretation and active-command refusal contract with deterministic static checks and dated forward-tests. Record official installation/discovery evidence for each claimed host, initially Codex, Claude Code, Cursor, and VS Code/Copilot; withhold unsupported or unverified claims. Add no new CLI behavior, target authority, direct MCP-server mode, auto-installation, permission grant, dependency, publication, or adoption claim. |
...
| MCPD-035 | Begin only after `MCPD-034` is Done. Create one repository-owned, vendor-neutral `mcp-doctor` Agent Skill and keep all behavior in its canonical `SKILL.md`; vendor-specific material may describe only installation and discovery. Add `docs/agents.md` and a README “Use with coding agents” route that state the current interaction is coding agent → terminal → `mcp-doctor` CLI → target MCP server, not an `mcp-doctor` MCP server configuration. Make the instruction-only skill check `mcp-doctor --version` and `mcp-doctor capabilities --format json`, stop with installation guidance rather than install software when the executable is absent, default to `inspect --format json`, require an exact user- or repository-selected command or endpoint, and interpret primary diagnosis, independent findings, causal skips, outcome, and exit without exposing raw values. Permit a code correction and rerun of the same passive inspection only when editing was requested. Never infer a target or escalate from discovery to `check`, `break`, or `reject`; declare no `allowed-tools` or equivalent permission grant; bundle no executable script; and never solicit, retain, or render secrets. Include a report-only CI handoff that reads an existing stable JSON artifact without rerunning the target. Revalidate primary vendor documentation at implementation time and record the exact product version, date, discovery path, and result for Codex, Claude Code, Cursor, and VS Code/Copilot; limit support text to the routes actually verified. Forward-test passive diagnose-only, explicitly requested fix-and-rerun, existing-report triage, and ambiguous-active refusal prompts, while treating these dated observations as integration evidence rather than a deterministic model-correctness or adoption claim. Add deterministic skill-structure, forbidden-permission/script, command-contract, documentation-link, and redaction checks; update the opt-in adoption-evidence route to record host and workflow without telemetry and without treating downloads, installs, stars, or vendor compatibility as independent usage. Add no CLI or stable-schema change, active authority, direct MCP mode, IDE extension, plug-in, daemon, SDK, hosted service, dependency, automatic installation, publication requirement, broad compatibility claim, or M4 reopening. Finish only when the complete locked local gate, skill/document checks, dated host matrix, protected exact-head checks, and durable scoped evidence pass. |
...
| RISK-31 | Coding-agent guidance guesses a target, escalates passive discovery into active calls, grants itself permissions, leaks secrets, or overstates host support or actual adoption | Critical | `DEC-057` and proposed `MCPD-035` require one canonical instruction-only skill, exact user- or repository-selected targets, compiled capability discovery, passive JSON inspection by default, explicit editing intent before correction and rerun, no inferred `check`/`break`/`reject`, no permission grant, installation, executable script, or secret handling, and dated exact-version evidence for every claimed host. The CLI's existing target, network, credential, tool, effect, side-effect, and cleanup gates remain authoritative even when agent instructions fail. | Open until `MCPD-035` passes its static safety checks and dated host forward-tests. Vendor compatibility, downloads, installs, and stars remain non-adoption evidence; any behavioral fork, inferred authority, unsupported host wording, secret exposure, or direct MCP-mode shortcut blocks completion and reopens the risk. |
