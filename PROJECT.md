...
| Product state | The passive local STDIO MVP, pinned current-revision compatibility matrix, bounded local and Streamable HTTP `check`, deterministic `break`, passive legacy inspection, current-revision contract snapshots and diffs, one-run JSON/JUnit report artifacts, conservative offline aggregation, structured unsupported-version diagnosis, and stable schema-backed JSON and JUnit-compatible projections pass local, protected hosted, immutable-release, and represented installed-channel evidence in `v0.3.0`; optional compiled-only capability discovery and explicit passive legacy-revision snapshot/diff support pass local and protected hosted source evidence without target replay, a new release, or a broad compatibility claim; completed `MCPD-027` source support adds exact-selected MCP `2025-11-25` `check` and `break` with local synthetic, controlled two-language STDIO, clean package/install, protected exact-head, and exact-`main` native CI, CodeQL, release-preflight, compatibility, and represented source-install evidence, while published active support and ecosystem-wide compatibility remain unclaimed |
| Current milestone | M4 — enterprise assurance and adoption; `MCPD-016A` is Done and `MCPD-017` is In progress |
| Overall status | M0 through M3 are Done; immutable `v0.1.0`, `v0.2.0`, and `v0.3.0` channels are verified; protected `main` retains the `MCPD-013` controls; the scoped `MCPD-014` repository-security baseline and revalidated three-repository `MCPD-015` public community, channel, and license contract pass with their explicit limitations; dated `MCPD-016` evidence and the completed `MCPD-016A` correction prove the current supply-chain boundary, including direct digest-authenticated Syft acquisition, the narrowed live Action allowlist, and first-attempt exact-head and exact-`main` gates; `DEC-041` and `DEC-042` fix the organization-access policy; owner-authorized `MCPD-017` activation has applied the supported member, App, OAuth, PAT, short-lived verifier, secure-2FA, credential-inventory, deploy-key, and private-recovery controls, with closure still withheld until the protected change merges and the non-disclosing verifier passes on exact `main`; optional `MCPD-021` through `MCPD-027` are Done for resolved issues #72, #73, #64, #66, #74, and #60 plus the independently verified `v0.3.0` release without changing the M4 gate |
| Current focus | Complete the `MCPD-017` exact-`main` closure independently; completed optional `MCPD-019` through `MCPD-027`, in-progress optional `MCPD-028`, and proposed optional `MCPD-029` remain outside the M4 dependency chain and cannot delay, redefine, or become prerequisites for `MCPD-017` or `MCPD-018` |
...
source scope. `MCPD-028` is in-progress optional work for issue #61;
`MCPD-029` remains proposed optional work for issue #75. `MCPD-027` established the shared active legacy boundary
that `MCPD-028` must consume; `MCPD-029` has no semantic dependency on legacy
support, but any overlapping active-engine integration must consume or rebase
onto that settled boundary rather than fork it. Planning and isolated fixture
work may run in parallel, while merge order must preserve those dependencies.
None is part of M4. The merged `MCPD-027` source changes the exact MCP
`2025-11-25` active-command and capability-manifest boundary under its scoped
evidence, but no published artifact or ecosystem-wide compatibility claim
follows. The other candidate tickets change no implemented revision,
snapshot, capability-manifest, or active-command claim before their own
...
- `check` and `break` default to MCP `2026-07-28`. `MCPD-027` added the exact
  `2025-11-25` value; `MCPD-028` may add only exact `2025-06-18` after applying
  its stricter schema policy. No range, alias other than the existing current
...
The in-progress `MCPD-028` source selects MCP `2025-06-18` through that same
...
and PowerShell source-install smokes now describe the same exact-selected
source capability. The complete local gate, `cargo-deny`, source review,
represented POSIX smoke, and retained controlled compatibility matrix pass;
deterministic packaged install, native hosted gates, protected merge, issue
closure, and exact-`main` evidence remain open before `MCPD-028` is Done.
...
| D-19 | Explicit MCP `2025-06-18` `check` and `break` | Optional compatibility | In progress | [Issue #61](https://github.com/EnjoyableWork/mcp-doctor/issues/61) depends on the `MCPD-027` foundation and adds a distinct no-guess schema-dialect gate; no support is claimed before `MCPD-028` is Done |
