...
| Product state | The passive local STDIO MVP, pinned current-revision compatibility matrix, bounded local and Streamable HTTP `check`, deterministic `break`, and stable schema-backed JSON and JUnit-compatible projections pass local, hosted, immutable-release, and represented installed-channel evidence; unreleased passive legacy inspection, contract snapshots and diffs, one-run JSON/JUnit report artifacts, and conservative offline aggregation pass protected implementation evidence, while the `v0.3.0` release candidate corrects structured unsupported-version diagnosis before publication |
| Current milestone | M4 — enterprise assurance and adoption; `MCPD-016A` is Done and `MCPD-017` is In progress |
| Overall status | M0 through M3 are Done; immutable `v0.1.0` and `v0.2.0` channels remain verified; protected `main` retains the `MCPD-013` controls; the scoped `MCPD-014` repository-security baseline and revalidated three-repository `MCPD-015` public community, channel, and license contract pass with their explicit limitations; dated `MCPD-016` evidence and the completed `MCPD-016A` correction prove the current supply-chain boundary, including direct digest-authenticated Syft acquisition, the narrowed live Action allowlist, and first-attempt exact-head and exact-`main` gates; `DEC-041` and `DEC-042` fix the organization-access policy; owner-authorized `MCPD-017` activation has applied the supported member, App, OAuth, PAT, short-lived verifier, secure-2FA, credential-inventory, deploy-key, and private-recovery controls, with closure still withheld until the protected change merges and the non-disclosing verifier passes on exact `main`; optional `MCPD-021` and `MCPD-022` are Done for resolved issues #44 and #45; and optional `MCPD-023` plus `MCPD-024` are In progress to correct issue #30 and publish the resulting `v0.3.0` feature release without changing the M4 gate |
| Current focus | Complete the `MCPD-017` exact-`main` closure independently; in parallel, finish `MCPD-023` causal protocol-rejection correction and then publish and independently verify `MCPD-024` as `v0.3.0`, without making either optional track a prerequisite for `MCPD-017` or `MCPD-018` |
| Public release | `mcp-doctor` `v0.2.0` remains the verified public release; `v0.3.0` is the reviewed release candidate and is not complete until GitHub, crates.io, Homebrew, and installed-channel evidence pass |
| Last reviewed | 2026-08-13 |
| Next review trigger | Any organization authentication, member, owner, billing-manager, invitation, base-permission, member-privilege, repository-creation, application, OAuth, personal-access-token, organization credential, in-scope repository credential, deploy-key, recovery, public-repository, community-route, official-channel, source/package/archive/formula license, security-policy, supported-line, entitlement, scan result, ruleset, merge-setting, administrator-boundary, required-context, GitHub-capability, Action, workflow, dependency proposal, report destination, projection fan-out, aggregate schema or outcome policy, tracked artifact, voluntary-usage, trusted-publisher, tap-authority, release-pipeline, testing-tool, safety-boundary, or assurance-evidence change |
...
`MCPD-023` is a focused optional correctness ticket running alongside M4. It
addresses GitHub issue #30 by recognizing only the current revision's exact,
...
dependency and cannot become a prerequisite for `MCPD-017` or `MCPD-018`.

`MCPD-024` is the intentional `v0.3.0` release track for completed optional
...
not publish or depend on an M4 assurance claim and cannot redefine the
...
| D-14 | Correct structured unsupported-version causal diagnosis | Optional correctness | In progress | `DEC-048`, the bounded transport classifier, shared diagnostic model, and focused passive, active, reporter-parity, no-replay, malformed-shape, and redaction tests are the local evidence; protected exact-head and exact-`main` gates, merge, issue #30 closure, and durable evidence remain open |
| D-15 | Immutable independently verified `v0.3.0` feature release | Optional release | In progress | `MCPD-024` prepares the reviewed version, release notes, supported line, portable README example, and exact release inputs; protected merge, annotated tag, GitHub/crates.io/Homebrew publication, and installed-channel verification remain open |
...
| MCPD-023 | Classify the exact current-revision unsupported-version response at the protocol layer | Optional correctness | In progress | `MCPD-010` | `DEC-048`; local focused passive, active, reporter-parity, exact-shape, malformed-shape, redaction, and no-replay evidence passes; protected exact-head and exact-`main` gates, merge, issue #30 closure, and durable evidence remain open |
| MCPD-024 | Publish and independently verify completed optional capabilities as `v0.3.0` | Optional release | In progress | `MCPD-023` | Reviewed release metadata, notes, supported line, portable README example, package, and all local release gates; then protected merge, annotated exact-`main` tag, immutable GitHub/crates.io/Homebrew byte handoffs, and represented installed-channel verification |
...
| Choice | Accepted policy |
| --- | --- |
| Supported versions | Support only the release candidate's minor line, `0.3.x`, as part of the same reviewed publication change. `0.2.x` and earlier are unsupported once `v0.3.0` publishes, and `main` is development-only without a release or backport guarantee. A report about an older version remains welcome, but upgrade may be the resolution. If publication does not complete, revert the candidate projection rather than claiming an unavailable supported line. |
