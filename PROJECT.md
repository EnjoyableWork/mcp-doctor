...
| Current focus | Complete the `MCPD-017` exact-`main` closure independently; completed optional `MCPD-019` through `MCPD-028` and proposed optional `MCPD-029` remain outside the M4 dependency chain and cannot delay, redefine, or become prerequisites for `MCPD-017` or `MCPD-018` |
...
source scope. `MCPD-029` remains proposed optional work for issue #75.
...
ecosystem-wide compatibility claim follows. The remaining candidate ticket
changes no implemented active-command claim before its own acceptance evidence
passes.
