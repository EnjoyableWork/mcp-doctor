...
reports on every represented path. The complete locked local gate, `cargo
deny`, package and installed-source checks, protected exact-head and exact-`main`
required checks, protected merge, issue closure, and durable completion evidence
are required before `MCPD-022` is Done; no publication is part of this ticket.
