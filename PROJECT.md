...
| Current focus | Complete the `MCPD-017` exact-`main` closure independently; completed optional `MCPD-019` through `MCPD-027` and proposed optional `MCPD-028` and `MCPD-029` remain outside the M4 dependency chain and cannot delay, redefine, or become prerequisites for `MCPD-017` or `MCPD-018` |
...
source scope. `MCPD-028` and `MCPD-029` remain proposed optional work for
issues #61 and #75. `MCPD-027` established the shared active legacy boundary
