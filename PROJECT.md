...
| Overall status | M0 through M3 are Done; immutable `v0.1.0`, `v0.2.0`, and `v0.3.0` channels are verified; protected `main` retains the `MCPD-013` controls; the scoped `MCPD-014` repository-security baseline and revalidated three-repository `MCPD-015` public community, channel, and license contract pass with their explicit limitations; dated `MCPD-016` evidence and the completed `MCPD-016A` correction prove the current supply-chain boundary, including direct digest-authenticated Syft acquisition, the narrowed live Action allowlist, and first-attempt exact-head and exact-`main` gates; `DEC-041` and `DEC-042` fix the organization-access policy; owner-authorized `MCPD-017` activation has applied the supported member, App, OAuth, PAT, short-lived verifier, secure-2FA, credential-inventory, deploy-key, and private-recovery controls, with closure still withheld until the protected change merges and the non-disclosing verifier passes on exact `main`; optional `MCPD-021` through `MCPD-029` are Done for resolved issues #72, #73, #64, #66, #74, #60, #61, and #75 plus the independently verified `v0.3.0` release without changing the M4 gate |
| Current focus | Complete the `MCPD-017` exact-`main` closure independently; completed optional `MCPD-019` through `MCPD-029` and active optional `MCPD-030` remain outside the M4 dependency chain and cannot delay, redefine, or become prerequisites for `MCPD-017` or `MCPD-018`; proposed `MCPD-031` is a prepublication gate for the next release only |
...
rejection-diagnostic scope. `MCPD-030` is active optional deterministic-CI
policy work for issue #41 under `DEC-054`. It audits and corrects test and
automation evidence only; it changes no product limit, runtime authority,
release artifact, live setting, or M4 gate. None is part of M4, and none may
...
| D-21 | Deterministic CI policy and timing-dependent evidence audit | Optional engineering assurance | In progress | `DEC-054`, `MCPD-030`, [issue #41](https://github.com/EnjoyableWork/mcp-doctor/issues/41), draft [PR 83](https://github.com/EnjoyableWork/mcp-doctor/pull/83), and exact implementation head [`6fb3232`](https://github.com/EnjoyableWork/mcp-doctor/commit/6fb3232782b0898c51c294efbb6c6a50296a8be5) own [the dated tracked audit](docs/deterministic-ci.md), the enforced timing/retry/watchdog/tool inventory, descendant-ready plus exclusive-lock-release cleanup proof, HTTP peer-close and client-completion handshakes, and exact runner-command verification in current hosted workflows. The isolated complete gate, `cargo-deny`, source-artifact review, actionlint, exact verifier execution, and deterministic package/generation/installed smoke pass locally on 2026-08-14 without changing a product timeout, safety invariant, dependency, release artifact, live setting, or M4 gate; first-attempt exact-head hosted evidence, protected merge, issue closure, exact-`main`, and durable completion evidence remain open |
...
| MCPD-030 | Adopt deterministic CI policy and audit timing-dependent tests and automation | Optional engineering assurance | In progress | `MCPD-003` | Under `DEC-054`, [issue #41](https://github.com/EnjoyableWork/mcp-doctor/issues/41), draft [PR 83](https://github.com/EnjoyableWork/mcp-doctor/pull/83), and exact implementation head [`6fb3232`](https://github.com/EnjoyableWork/mcp-doctor/commit/6fb3232782b0898c51c294efbb6c6a50296a8be5) now have a complete tracked test/script/workflow audit; explicit product-bound, product-clock, outer-watchdog, deterministic-state, acquisition-retry, runner-tool, and defect classifications; direct exclusive-lock-release and peer-close cleanup evidence; current hosted runner verification; enforced inventories; preserved historical failures; and proposed `MCPD-031` ownership for release-only retry, polling, and legacy runner checks. The isolated complete gate, `cargo-deny`, source-artifact review, actionlint, POSIX/PowerShell verifier execution, and deterministic package/generation/installed smoke pass locally on 2026-08-14 without retry, quarantine, timeout inflation, assertion weakening, broad serialization, a dependency, or a product/runtime/release/live-setting/M4 change. First-attempt exact-head hosted checks, protected merge, issue closure, exact-`main`, and durable completion evidence remain open. |
...
| RISK-27 | Scheduler-dependent fixtures, correctness retries, or incidental runner tools make identical source pass and fail or hide a safety regression | High | `DEC-054` and `MCPD-030` require a complete tracked timing/tool inventory; product bounds only in product code or explicit limit fixtures; audited product-clock input only for a contract that consumes verification time; job, socket, and channel deadlines only as outer watchdogs; observable state or event/acknowledgement fixture coordination; deterministic forced-state regressions; one exact immutable Syft acquisition exception under `DEC-043`; declared non-standard CI tools; preserved failed evidence; and first-attempt acceptance without rerun, timeout inflation, assertion weakening, quarantine, or broad serialization | Open while `MCPD-030` is In progress. The prior organization-control runner-tool and fixture-date defects were corrected in PR #40, and the retained 2026-08-14 identical-source cleanup variance is now corrected by a forced readiness plus exclusive-lock-release proof that passes the complete local gate. Current CI, compatibility, and release-preflight source jobs verify exact declared runner commands; protected native exact-head evidence remains open. Release-only retry, eventual-state, and legacy runner-verification mechanisms cannot provide a later publication claim until proposed `MCPD-031` corrects and rehearses them. |
