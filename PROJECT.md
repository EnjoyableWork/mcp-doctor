...
| Overall status | M0 through M3 are Done; immutable `v0.1.0` and `v0.2.0` channels remain verified; protected `main` retains the `MCPD-013` controls; the scoped `MCPD-014` repository-security baseline and revalidated three-repository `MCPD-015` public community, channel, and license contract pass with their explicit limitations; dated `MCPD-016` evidence and the completed `MCPD-016A` correction prove the current supply-chain boundary, including direct digest-authenticated Syft acquisition, the narrowed live Action allowlist, and first-attempt exact-head and exact-`main` gates; `DEC-041` and `DEC-042` fix the organization-access policy; and staged owner-authorized activation has applied supported member and App-installation controls and removed the obsolete tap deploy key without claiming that App scope, credential policy and migration, private recovery, or the final verifier have passed |
| Current focus | Complete the remaining `MCPD-017` App, credential, authority, inventory, and private-recovery activation, then bind its exact-`main` evidence |
...
| D-10 | Explicit passive legacy-revision diagnostics | Optional compatibility | In progress | `MCPD-019` implements revision-specific synthetic STDIO and Streamable HTTP evidence; broad real-server wording remains gated on the controlled two-language matrix and represented installed-channel journeys |
...
| MCPD-019 | Add explicit passive `inspect` diagnostics for MCP `2025-11-25` and `2025-06-18` over STDIO and Streamable HTTP | Optional compatibility | In progress | `MCPD-012` | `DEC-044`; exact opt-in CLI and selected/negotiated reporting; revision-specific lifecycle, catalogs, pagination, schema, header, optional-session, request-scoped SSE, and teardown behavior; synthetic positive and negative built-binary journeys for both revisions and transports; stable human/JSON/JUnit parity; complete locked gates; and no broad real-server wording before the controlled official/independent two-language and represented installed-channel evidence passes |
...
incomplete, the live verifier fails closed, and `MCPD-017` is In progress after
`MCPD-016A` restored the release-preflight boundary.
