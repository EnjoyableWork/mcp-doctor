...
| MCPD-003 | Add disposable local gates, dependency policy, least-privilege cross-platform CI, maintenance automation, and community/security entry points | M0 | Done | `MCPD-002` | POSIX and PowerShell gates, `cargo-deny`, exact direct-requirement enforcement, dated dependency and testing-tool reviews, Actionlint, ShellCheck, YAML parsing, links, packaging, and identity checks pass locally; the [first hosted matrix on merged `main` `f788e76`](https://github.com/EnjoyableWork/mcp-doctor/actions/runs/31337295110) passes the original baseline and the policy refinement awaits its own hosted run |
