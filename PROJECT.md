...
| D-26 | Bounded SSE decoding and actual JSON Schema work | Security correction | In progress | `DEC-060`, `DEC-061`, `DEC-062`, and `MCPD-036`; local verification confirms the fragmented-SSE regression in `v0.3.0` through `v0.3.2` and the schema-work accounting gap in `v0.1.0` through `v0.3.2`. Completion requires independent deterministic work-bound regressions, complete local and represented native gates, the corrected historical-distribution and read-only repository-setting operator audit, one immutable patched release, installed-channel verification, and coordinated publication of both corrected advisories. |
...
`DEC-061` records the historical-versus-rolling release-gate correction, and
`DEC-062` records the least-authority merge-setting observation correction
discovered during `MCPD-036`.
