...
| MCPD-034 | Bind authority-bearing local inputs to the exact regular file opened for the diagnostic | Security correction | In progress | `MCPD-010`, `MCPD-020`, `MCPD-022`, `MCPD-033` | Open scenario, custom-CA, snapshot, and aggregate inputs through one no-follow native-identity boundary; derive every size and regular-file decision from the opened handle; read only that handle; and fail closed when the selected path no longer identifies it. Deterministic forced-replacement, symlink/reparse, disappearance, oversize, hard-link/native-identity, no-target, no-connection, no-credential, and no-accepted-diff regressions must pass on represented native platforms together with the complete locked, dependency, package, and installed-source gates. Coordinate any released correction through the private disclosure process before recording public completion evidence. |
...
| MCPD-034 | Correct authority-bearing local input identity without public pre-disclosure: replace every path-precheck/following-open sequence for scenarios, custom CA trust, snapshots, and aggregate reports with one focused no-follow native-identity opener; make opened-handle metadata authoritative; verify the selected path still identifies that handle before reading; and fail closed without target start, network connection, credential resolution or transmission, or accepted snapshot evidence on replacement, disappearance, symlink/reparse substitution, non-regular input, or identity failure. Preserve all existing byte, certificate, schema, authorization, redaction, cleanup, and reporter contracts; add deterministic event/acknowledgement interleavings and native Unix/Windows identity evidence; add no retry, overwrite authority, raw path/value output, or unreviewed dependency; and finish only after private exact-head verification, coordinated patched release, advisory publication, exact-`main` gates, and a non-sensitive durable completion record. |
