...
| D-20 | Bounded schema-invalid tool-argument rejection diagnosis | Optional active correctness | In progress | `DEC-053` resolves `OPEN-16`; the working tree implements the current-only `reject` surface and focused synthetic evidence for [issue #75](https://github.com/EnjoyableWork/mcp-doctor/issues/75), and the complete local and dependency gates, two byte-identical dirty-tree package runs, packaged POSIX install smoke, release-binary POSIX smoke, and PowerShell parser pass, while native Windows execution, hosted checks, protected merge, issue closure, and durable exact-`main` evidence remain open and no published artifact claim follows |
