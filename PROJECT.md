...
`MCPD-034` privately batches exactly two confirmed native-identity corrections
into the coordinated `v0.3.1` candidate under `DEC-058`: authority-bearing
local inputs and report artifact publication. The quadratic SSE and schema
evaluation drafts remain separate work and cannot delay or broaden this patch.
...
| D-22 | Deterministic release-state, retry, and runner-tool correction | Optional release maintenance | In progress | `MCPD-031` removes every broad release curl retry and fixed-sleep publication loop, makes exact runner verification precede release evidence, and requires direct bounded immutable-state and byte-identity checks plus a nonpublishing rehearsal before any later public release, without changing immutable `v0.1.0`, `v0.2.0`, or `v0.3.0` bytes |
...
| MCPD-034 | Bind authority-bearing local inputs and report artifact publication to exact opened file identities | Security correction | In progress | `MCPD-010`, `MCPD-020`, `MCPD-022`, `MCPD-033` | `DEC-058`; open scenario, custom-CA, snapshot, and aggregate inputs through one no-follow native-identity boundary; derive every input decision and read from the opened handle; bind JSON, JUnit, and aggregate publication to the opened stage identity before linking and the destination identity immediately afterward; and remove only identity-owned stages or outputs during cleanup and rollback. Deterministic forced replacement, symlink/reparse, disappearance, oversize, hard-link/native-identity, no-target, no-connection, no-credential, no-accepted-diff, no-foreign-delete, two-output rollback, and no-aggregate-evidence regressions must pass on represented native platforms together with the complete locked, dependency, package, and installed-source gates. Coordinate both released corrections through one private `v0.3.1` disclosure while excluding the SSE and schema-work drafts, then record public completion evidence. |
