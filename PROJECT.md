...
`MCPD-035` is in-progress optional adoption-UX work now that
`MCPD-034` is Done. It packages the existing CLI and stable-report behavior as
one concise vendor-neutral Agent Skill, one immutable versioned companion
artifact, and one canonical coding-agent guide. The next patch release exposes
one terse installed-CLI help route to the explicit, reversible skill install;
no package channel silently modifies an agent host. Host-specific material is
limited to dated, exact-version installation, discovery, explicit-invocation,
and separately labeled implicit-invocation evidence. Initial eligibility covers
Codex, Claude Code, Cursor, VS Code/Copilot, Kiro IDE, Kiro CLI, and only Kiro
Crew's interactive foreground workflow; scheduled, proactive, webhook,
heartbeat, App, synthesized-skill, and other unattended Crew behavior remains
unclaimed. The ticket adds no product execution authority, direct MCP-server
mode, IDE extension, daemon, SDK, or hosted service.
...
| D-25 | Safe portable coding-agent diagnostic workflow | Optional adoption UX | In progress | `DEC-057` as superseded by `DEC-059`, and `MCPD-035`, require one canonical instruction-only Agent Skill, an immutable version-bound companion artifact, explicit reversible host installation, an installed-CLI help route, `docs/agents.md`, a README route, a dated exact-version host-support matrix, deterministic structure and safety checks, synthetic-recorder passive and refusal forward-tests, represented installed-channel evidence, and an opt-in adoption-evidence route before any coding-agent support or usage claim |
...
| MCPD-035 | Make the existing passive diagnostic workflow discoverable and safe for coding agents | Optional adoption UX | In progress | `MCPD-034`, `MCPD-025` | `DEC-057` as superseded by `DEC-059`; add one canonical vendor-neutral Agent Skill, an immutable version-bound companion artifact, explicit reversible host installation and removal, one terse installed-CLI help route, `docs/agents.md`, and a README route. Prove its passive-first, exact-target, capability-aware, redacted-report interpretation and active-command refusal contract with deterministic static checks, a synthetic command recorder, CLI safety-backstop regressions, and dated clean-profile forward-tests that distinguish explicit invocation from best-effort implicit selection. Record primary-source installation and discovery evidence separately for each claimed exact host/version, initially Codex, Claude Code, Cursor, VS Code/Copilot, Kiro IDE, Kiro CLI, and Kiro Crew interactive foreground sessions; withhold unsupported, unverified, scheduled, proactive, webhook, heartbeat, App, synthesized-skill, and other unattended Crew claims. Publish the matching skill and installed help route only through the next protected patch release and represented channel verification. Add no active target authority, direct MCP-server mode, silent installation, permission grant, dependency, marketplace-dominance, universal-agent, deterministic-model-correctness, or adoption claim. |
...
found every selected direct version stable, every declared upstream repository
active and not archived, the locked graph free of known advisories under
`cargo-deny`, and all selected licenses accepted or narrowly documented.
...
| RISK-31 | A binary-only user never discovers the companion skill, or a coding agent fails to load it, guesses a target, escalates passive discovery into active calls, grants itself permissions, leaks secrets, persists unsafe guidance, or overstates host support or actual adoption | Critical | Superseding `DEC-059` and in-progress `MCPD-035` require one canonical instruction-only skill and immutable version-bound archive, a terse installed-CLI help route, explicit reversible no-overwrite host installation, exact user- or repository-selected targets, compiled capability discovery, passive JSON inspection by default, explicit editing intent before correction and rerun, no inferred `check`/`break`/`reject`, no permission grant, executable script, automatic installation, or secret handling, and dated exact-version discovery plus explicit-invocation evidence for every individual claimed host. A synthetic command recorder proves the permitted sequence without real activity; direct CLI regressions keep the existing target, network, credential, tool, effect, side-effect, cleanup, and reporter gates authoritative when the skill is absent or ignored. Kiro Crew is a separate persistent surface restricted initially to a disposable interactive foreground session with unchanged skill identity and no unattended claim. | Open until `MCPD-035` passes deterministic skill/distribution/help and CLI-backstop checks, publishes and verifies the matching next-patch skill and installed help route, and records the dated exact-version host matrix. Implicit model selection remains best-effort rather than guaranteed; vendor compatibility, downloads, binary or skill installs, stars, and forward-tests remain non-adoption evidence. Any binary/skill conflation, silent install, canonical-byte drift, behavioral fork, inferred authority, unsupported or universal host wording, Crew persistence/background overclaim, secret exposure, or direct MCP-mode shortcut blocks completion and reopens the risk. |
