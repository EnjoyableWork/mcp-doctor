...
| Current focus | M4 is complete with annual and event-driven claim review active; proposed `MCPD-031` remains the prepublication gate for the next release, while completed optional `MCPD-019` through `MCPD-030`, `MCPD-032`, and `MCPD-033` remain outside the closed M4 dependency chain |
